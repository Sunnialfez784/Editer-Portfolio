      ...
      {open && (
        <motion.div className="fixed inset-0 z-[95] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} onClick={onClose}>
          <motion.form onSubmit={submit} onClick={(e) => e.stopPropagation()} initial={{opacity: 0, scale: 0.92, y: 20}} animate={{opacity: 1, scale: 1, y: 0}} exit={{opacity: 0, scale: 0.92, y: 20}} transition={{duration: 0.3, ease: [0.22, 1, 0.36, 1]}} className="w-[calc(100vw-2rem)] max-w-md card p-6 sm:p-7 relative max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            {initial && !form.file && form.existingVideo ? (
              <div className="mb-3 rounded-xl overflow-hidden border border-white/10 bg-black/30">
                <video
                  src={form.existingVideo}
                  controls
                  preload="metadata"
                  className="w-full aspect-video object-contain bg-black"
                />
                <div className="flex items-center justify-between gap-3 px-3 py-2.5 border-t border-white/10">
                  <div className="min-w-0">
                    <p className="text-xs text-emerald-300">Current video selected</p>
                    <p className="text-xs text-mist-500 truncate">
                      {form.fileName || "Existing video"}
                    </p>
                  </div>
                  <label className="shrink-0 cursor-pointer rounded-lg bg-white/5 hover:bg-white/10 px-3 py-2 text-xs text-mist-200 transition-colors">
                    Change Video
                    <input
                      type="file"
                      accept={VIDEO_ACCEPT}
              ...
              <input
                ...
                accept={VIDEO_ACCEPT}
                onChange={handleFile}
                className="w-full mb-3 text-sm text-mist-400 file:mr-4 file:rounded-full file:border-0 file:bg-white/5 file:px-4 file:py-2.5 file:text-mist-100 hover:file:bg-white/10"
              />
            )}
            ...
            <p className="text-xs text-mist-500 mb-4">
              {initial
                ? form.file
                  ? `New video selected: ${form.fileName}`
                  : "The existing video is already selected. Choose Change Video only if you want to replace it."
                : "MP4 or WEBM."}
            </p>
            ...
            ) : form.fileName && !form.existingVideo ? (
